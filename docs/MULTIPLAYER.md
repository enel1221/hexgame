...
Open the game in two tabs. In the first, create a room, copy the six-character code, and keep the reconnect token in browser storage. In the second, join that code, mark the guest ready, then start from the host tab. Closing and reopening the second tab should reconnect with the stored token and receive a checkpoint plus any commands after it.
...
Create/join returns a room code, stable player ID, assigned lobby seat, reconnect token, room configuration, and a ready-to-use WebSocket URL. Room codes use a 32-character alphabet without `I`, `O`, `0`, or `1`. Seats are compacted if someone explicitly leaves before start, then become immutable when the match starts.
...
- `start`: host-only; requires at least two active humans and all humans ready.
...
- `started`: immutable match configuration and seat mapping.
...
If a browser has already published an authoritative target tick before a delayed batch reaches it, its Worker does not clamp that command to a later local tick. It rolls back to a bounded pre-target snapshot, replays accepted and pending relay commands in receipt order through its prior current tick, and republishes the corrected deterministic state. The Worker retains one recovery base plus four recent 50-tick points; the base also permits a longer replay when a tab was delayed beyond the recent window.
...
On connection during a running or completed match, the room sends:
